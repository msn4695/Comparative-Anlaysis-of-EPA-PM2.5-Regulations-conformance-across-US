...
yeardata <- with(NEI, tapply(Emissions, year, sum))
yeardata1 <- data.frame(Year = names(yeardata), Total_Emissions = yeardata)
yeardata1$Year <- as.integer(yeardata1$Year)

# Plotting data

png("plot.png", height = 461, width = 762, units = "px")
with(yeardata1, plot(Year, Total_Emissions, pch = 19, 
                     main = "Total Emissions from 1999 to 2008",
                     xlab="Year", ylab="Emissions (tons)"),
)
lmm <- lm(Total_Emissions ~ Year, yeardata1)
abline(lmm, lwd = 2)
